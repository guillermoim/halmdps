...
def solve(env, iters=10000, log=False):
 
    IS, TS, _, P, R = env.get_LMDP()

    # Run RVI
    z, gain = exp_rvi(IS + TS, P, R, iters=iters)

    # Get eigenvectors
    w, v = np.linalg.eig(np.diag(np.exp(-R)) @ P)
    gammaeig = np.max(w)
    zeig = v[:, np.argmax(w)]
...
if __name__ == "__main__":
    ...
    Z, gamma, zs = solve(env, log=True)

    os.makedirs("../results/ground_truth/", exist_ok=True)

    with open(f"../results/ground_truth/{env_name}.pkl", "wb") as fp:

        pkl.dump((Z, gamma, zs), fp)